defmodule ReqLLM.Step.Usage do
  ...
  * Metadata: `%{model: %LLMDB.Model{}}`
  ...
  defp normalize_usage(usage) when is_map(usage) do
    # Detect API format FIRST - this determines how to interpret token counts
  ...
  defp get_cached_input_tokens(usage, input, input_includes_cached) do
    ...
    if input_includes_cached do
      # OpenAI: input includes cached, so cached must be <= input
      clamp_tokens(cached, input)
    else
      # Anthropic: input excludes cached, no constraint between them
      safe_to_int(cached)
    end
  end
  ...
  defp get_cache_creation_tokens(usage, input, input_includes_cached) do
    ...
    if input_includes_cached do
      # OpenAI: input includes cached, so creation must be <= input
